...
Gesture → Action mapping (enforced in actions.py):
    Pointing      → Mouse move       (index only)
    Two Fingers   → Left click       (index + middle)
    Three Fingers → Scroll mode      (index + middle + ring)
    Four Fingers  → Open Chrome      (index + middle + ring + pinky)
    Fist          → Play / Pause
    Thumbs Up     → Volume Up
...
#
# Primary action gestures (must match ACTION_COOLDOWNS keys in config.py):
#   Pointing      (False, True,  False, False, False) → mouse move
#   Two Fingers   (False, True,  True,  False, False) → left click
#   Three Fingers (False, True,  True,  True,  False) → scroll
#   Four Fingers  (False, True,  True,  True,  True)  → open Chrome
#   Fist          (False, False, False, False, False)  → play/pause
#   Thumbs Up     (True,  False, False, False, False)  → volume up (refined below)
#   Open Palm     (True,  True,  True,  True,  True)   → screenshot
#
_GESTURE_TABLE: dict[tuple[bool, ...], str] = {
    # ── Primary action gestures ──────────────────────────────────
    (False, False, False, False, False): "Fist",
    (True,  False, False, False, False): "Thumbs Up",     # refined → Down below
    (False, True,  False, False, False): "Pointing",
    (False, True,  True,  False, False): "Two Fingers",   # left click
    (False, True,  True,  True,  False): "Three Fingers", # scroll
    (False, True,  True,  True,  True):  "Four Fingers",  # open Chrome
    (True,  True,  True,  True,  True):  "Open Palm",     # screenshot

    # ── Inert / informational gestures ──────────────────────────
...
def recognize(hand: HandResult) -> GestureResult:
    ...
    A low confidence score means the hand shape is ambiguous and the
    gesture should be ignored (see ``GESTURE_CONFIDENCE_THRESHOLD``).
